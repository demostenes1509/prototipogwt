...
public class DBCPConnectionProvider implements ConnectionProvider {
    ...
    private boolean askedForClosing = false;
    ...
    public DBCPConnectionProvider() {
    	log.info("Creating DBCPConnectionProvider");
    }
        ...
        try {
            ...
            for (Iterator iter = props.entrySet().iterator() ; iter.hasNext() ;) {
                ...
                if (entry.getKey().toString().startsWith(PREFIX)) {
                    String property = entry.getKey().toString().substring(PREFIX.length());
                    String value = (String) entry.getValue();
            ...
            if (ds != null) {
                try {
                    ...
                    log.debug("No problem if close fails");
                }
                ds = null;
            }
    ...
    public Connection getConnection() throws SQLException {
        ...
        try {
        		if(ds!=null) {
        			if(askedForClosing==true) {
        				log.warn("DBCP was asked to close Connection before !!");
        			}
...
//                ds.close();
//                    ds = null;
            	log.info("ORM asked for closing DataSource");
            }
            else {
